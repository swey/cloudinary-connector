...
class CloudinaryConnector {
  ...
  async getBreakpoints(imageUrl, transformation, breakpointConfig = null) {
    const {
      ...
      maxBreakpoints,
      list: breakpointList
    } = breakpointConfig ? _objectSpread({}, this._breakpointConfig, breakpointConfig) : this._breakpointConfig; // If a "list" is given in the breakpoint config, use that list instead of calculate one.
